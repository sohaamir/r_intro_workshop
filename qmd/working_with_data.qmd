...
In the code below, we are downloading the data from the internet and saving it to our working directory. We can then read it in using `read.csv`. The first two lines are specific to our online implementation of code; in `R` you can just run the bottom line.
...
Here we are using the `ggplot2` package using the `geom_point` function to create the points, and the `geom_smooth` function to add a line of best fit. The `method = "lm"` argument specifies that we want to use a simple linear regression model for the line of best fit.
...
In the case that we want to see whether there is a significant relationship between two continuous variables, i.e., between age and lesion volume across the entire group, we can run a correlation. We can do this using the `cor.test` function, which will give us the correlation coefficient (r) and the corresponding p-value.
...
And we can also directly plot this using `ggplot`:
...
data_removed <- data[data$lesion_volume <= 5, ]
...
data_removed <- subset(data, lesion_volume <= 5)
```
...
We can now see that there is only a significant difference in the relationship between age and total lesion volume for females, and not for males. Regardless, by looking at the data in this way we have found something interesting (which we didn't see before) that we can subsequently look at further.

## Regression models

In the HAVEN study, we were interested with understanding the potential impact of demographic and brain factors upon behaviour. To do this we can look at participants' reaction times and accuracy during an episodic memory task using the `rt_data` dataset.
...
It is essentially an expanded version of the previous dataset, which included accuracy and mean reaction time for correct and incorrect responses.

Our goal is to create a model to describe how various factors influence participants' performance on the task.

But first, lets plot the relationship between age and performance on the task, both for the reaction time (only for ones they got correct) and the percentage accuracy.

:::{.callout-tip title="Look at your data"}
Before running any modelling analyses, it is always a good idea to look at your data and it's relationships, even it is rudimentary!
:::

We can do this using `ggplot2` and the `lm` function to create a simple linear model. We will also add a line of best fit to the plot, and annotate it with the p-value from the model.
```{webr-r}
#| context: interactive
...
Linear regression helps us understand how different factors ("predictors") influence an outcome ("dependent variable"). In our case, we want to see how age, sex, and education affect task performance (reaction time and accuracy).

Let's start with one predictor - age:
```{webr-r}
#| context: interactive

# Simple regression model
model1 <- lm(correct_rt ~ Age, data = rt_data)

# View results
summary(model1)
```

Now let's add sex and education:
```{webr-r}
#| context: interactive

# Multiple regression model
model2 <- lm(correct_rt ~ Age + Sex + education, data = rt_data)
...
## Calculating group differences using ANOVA 

We know that there is a correlation between age and lesion volume, but is there a difference between groups? We can use ANOVA to test this.

Firstly, let's see the summary statistics for Males and Females:
...
sex_stats <- rt_data %>%
  ...
  summarise(
    n = n(),
    mean_rt = mean(correct_rt, na.rm = TRUE),
    sd_rt = sd(correct_rt, na.rm = TRUE),
    mean_acc = mean(correct_percent, na.rm = TRUE),
    sd_acc = sd(correct_percent, na.rm = TRUE)
  )
...
rt_anova <- aov(correct_rt ~ Sex, data = rt_data)
...
#| context: interactive
# ANOVA for accuracy
acc_anova <- aov(correct_percent ~ Sex, data = rt_data)
