...
So you can see we have a variety of different variables, including `age`, `sex`, `height`, `weight`, `bmi` and `lesion_volume`.
...
If we wanted to understand whether there is a difference between the height and weight between males and females, we can also do this using a scatter plot, but grouped by sex:
```{webr-r}
#| context: interactive

ggplot(sub_data, aes(x=height, y=weight, colour=sex)) + 
  geom_point() +
    labs(x = "Height (m)",
         y = "Weight (kg)") +
  theme_minimal()
...
ggplot(sub_data, aes(x = sex, y = height, fill = sex)) +
...
# Plot 2: Weight by Sex
ggplot(sub_data, aes(x = sex, y = weight, fill = sex)) +
  geom_boxplot(width = 0.3) +
  stat_summary(fun = mean, geom = "point", shape = 18, size = 4, color = "red") +
  labs(x = "Sex",
       y = "Weight (kg)") +
...
To answer these questions, lets firstly get some more summary statistics, this time for height and weight specificially. We firstly group the data by sex and run the `summarise` function to calculate the mean and standard deviation for each group.

```{webr-r}
#| context: interactive

group_by(sub_data, sex) %>%
  summarise(
    count = n(),
...
# filter the data
males <- sub_data[sub_data$sex == "M", ]
females <- sub_data[sub_data$sex == "F", ]
...
model_male <- lm(`lesion_volume` ~ age, data = sub_data_filtered[sub_data_filtered$sex == "M",])
model_female <- lm(`lesion_volume` ~ age, data = sub_data_filtered[sub_data_filtered$sex == "F",])
...
ggplot(sub_data_filtered, aes(x = age, y = lesion_volume, color = sex)) +
  geom_point() +
...
:::{.callout-info title="The episodic memory task"}
...
Let's firstly change the column names to be lowercase using the `tolower()` function - this makes it easier to work with!
```{webr-r}
#| context: interactive

names(rt_data) <- tolower(names(rt_data))
names(rt_data)
```
...
## Calculate correlation and p-values for each sex
male_data <- rt_data[rt_data$sex == "M", ]
female_data <- rt_data[rt_data$sex == "F", ]

male_corr <- cor.test(male_data$age, male_data$correct_rt)
female_corr <- cor.test(female_data$age, female_data$correct_rt)

# Create plot with separate colors and regression lines for each sex
percent_plot_by_sex <- ggplot(rt_data, aes(x = age, y = correct_rt, color = sex, fill = sex)) +
...
The `lm` function - as previously used - creates a linear model, and the `str` function shows the structure of the model. You can see that a lot happens underneath the hood! Importantly, we can use the `summary` function to see the results of the model, including the coefficients, p-values and R-squared value.
...
To specifically test the influence of age and sex on performance - as indicated by the correlation plot, we need to add sex to our model. But there are a few things to note:

1) Because sex is a categorical variable, it should be implemented in the model as a factor. We actually do not need to do this explicitly, as the `lm` function will automatically convert it to a factor. However, it is good practice to do so, so we will do it explicitly as well.
2) We should add sex as an interaction term with age, rather than a separate fixed effect. This is because we are explicitly interested in whether two slopes are significantly different. Adding both as separate terms would just test the effect of each separately, and not the interaction.

Here is the code to create and run this model:
```{webr-r}
#| context: interactive

# Convert sex to factor (not explicitly needed)
rt_data$sex <- factor(rt_data$sex, levels = c("F", "M"))

# Multiple regression model - * indicates interaction
age_sex_model <- lm(correct_rt ~ age * sex, data = rt_data)
...
anova(age_model, age_sex_model)
```

We can see from the output that compared to the simple age model, adding in sex as an interaction significantly improves the model fit (p = 0.035).

:::{.callout-info title="ANOVA for model comparison"}
...
aic_age_sex <- AIC(age_sex_model)

bic_age <- BIC(age_model)
bic_age_sex <- BIC(age_sex_model)
```
