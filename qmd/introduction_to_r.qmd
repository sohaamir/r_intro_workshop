...
If the code fails to run or throws up an error, it is most likely because it relies upon variables that were set in a previous chunk which were not run. Please make sure that you run all of the chunks in order!
:::
...
Some example using the `students` dataframe:
```{webr-r}
#| context: interactive
...
These are done using built-in functions in `R` including `var()`, `sd()`, `sum()`, `mean()`, `min()`, and `max()`.
```{webr-r}
#| context: interactive
